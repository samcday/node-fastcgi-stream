...
var writeBuffer = new Buffer(constants.HEADER_SIZE + constants.MAX_CONTENT_SIZE + constants.MAX_PADDING_SIZE);
...
var FastCGIStream = module.exports = function(stream) {
	...
	var processIncomingData = function() {
		...
		if(type && (bufferList.length >= (contentLength + paddingLength))) {
			...
			if(contentLength) {
				var meh = bufferList.take(contentLength);
				console.log(meh);
				record.read(bufferList.take(contentLength));	
			}
	...
	this.writeRecord = function(requestId, record) {
		...
		stream.write(writeBuffer.slice(0, 8 + recordBodyLength + paddingLength));
	};
};
